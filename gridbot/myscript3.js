...
function girdiler() {
  ...
  let divisionResults = geometric_division(start, end, num_levels);
...
function ilkveonbakiyehesapla() {
    ...
    let levels, geometric_percentage;
...
Gridden elde edilen realized USDT: ${parseFloat((gridprofit * girilenboluminbakiye).toFixed(2))} USDT
Gridden yüzde kazanç: %${parseFloat(((gridprofit / yavilkbakiyeiste) * 100).toFixed(2))}
Grid'den aylık yüzde kazanç: %${parseFloat((30 * ((gridprofit / yavilkbakiyeiste) * 100) / calculate_days_between(start_date, end_date)).toFixed(2))}
Grid'den yıllık yüzde kazanç: %${parseFloat((12 * 30 * ((gridprofit / yavilkbakiyeiste) * 100) / calculate_days_between(start_date, end_date)).toFixed(2))}
Şuanki gridden gelen hariç bakiye: ${parseFloat((yavsonbakiyeiste * girilenboluminbakiye).toFixed(2))} USDT
Şuanki grid dahil bakiye: ${parseFloat(((yavsonbakiyeiste + gridprofit) * girilenboluminbakiye).toFixed(2))} USDT
Şuanki grid dahil bakiye bölü ilk bakiye: ${parseFloat(kazancorani.toFixed(2))} X (yani toplam %${parseFloat((100 * (kazancorani - 1)).toFixed(2))} ekstra gelir gelmiş.)
...
export function mybacktest(symbolg, startg, endg, num_levelsg, comissiong, girilenilkbakiyeg, veriintervalg, start_dateg, end_dateg) {
    /**
