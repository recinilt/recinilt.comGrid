...
function disableInteractions() {
    ...
    overlay.style.zIndex = '9999'; // Üstte olması için yüksek bir z-index
...
function hesaplabutton() {
    ...
    var result="";
    setTimeout(() => {
        result = mybacktest(symbol.value.toUpperCase().trim().replace("İ","I"), start.value.trim(), end.value.trim(), num_levels.value.trim(), comission.value.trim(), girilenilkbakiye.value.trim(), veriinterval.value.toLowerCase().trim(), start_date.value.trim(), end_date.value.trim());
...
<li>Kaç gündür bot çalışıyor: ${calculate_days_between(start_date, end_date)} gün</li>
<li>Gridler arası yüzde: %${geometric_percentage}</li>
...
<li>Gridden yüzde kazanç: %${parseFloat(((gridprofit / yavilkbakiyeiste) * 100).toFixed(2))}</li>
<li>Grid'den aylık yüzde kazanç: %${parseFloat((30 * ((gridprofit / yavilkbakiyeiste) * 100) / calculate_days_between(start_date, end_date)).toFixed(2))}</li>
